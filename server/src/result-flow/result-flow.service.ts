...
export class ResultFlowService {
    ...
    async updateLessonTest(id: string, dto: ResultTestDto) {
        ...
        const isExtendedTest = test.questions.some(question => question.is_extended)
            ...
            test.questions.forEach((question) => {
                if (!question.is_extended) {
                    ...
                    if (question.is_multiple && Array.isArray(response)) {
                        const arr = question.answers.filter(answer => response.findIndex(r => r === answer.id) !== -1)
                        if (arr.every(answer => answer.is_correct)) {
                            mark++
                        }
                    } else {
                        const answer = question.answers.find(answer => answer.id === +response)
    ...
    async setMarkTest(id: string, dto: ResultTestDto) {
        ...
        test.questions.forEach((question) => {
            ...
            if (!question.is_extended) {
                ...
                if (typeof response === 'object' && !Array.isArray(response) && response.isCorrect) {
                    mark++
                }
            }
        })
